...
// Proxy endpoint for PERMANENT DELETE (removes file from Google Drive and row from Google Spreadsheet)
app.post("/api/storage/delete", async (req, res) => {
  const { storageProfile, item, business_id, drive_file_id, file_name, user_name } = req.body;
  ...
  try {
    ...
    const response = await fetch(storageProfile.apps_script_url.trim(), {
      ...
      body: JSON.stringify({
        ...
        spreadsheetId: (storageProfile.spreadsheet_id || "").trim(),
        driveFolderId: (storageProfile.drive_folder_id || "").trim(),
        business_id: business_id || item?.business_id || item?.id,
        drive_file_id: drive_file_id || item?.drive_file_id || item?.file_id,
        file_name: file_name || item?.file_name,
        title: item?.title || file_name,
    ...
    try {
      ...
      return res.status(502).json({
        success: false,
        message: "Google Apps Script mengembalikan respons non-JSON saat menghapus berkas.",
      });
    }
    ...
    return res.status(500).json({
      success: false,
      message: `Gagal menghapus berkas dari Google Drive/Spreadsheet: ${err.message}`,
    });
  }
});

// Proxy endpoint for TRASH status update in Google Spreadsheet
app.post("/api/storage/trash", async (req, res) => {
  const { storageProfile, item, business_id, targetStatus } = req.body;
  ...
  try {
    ...
    const response = await fetch(storageProfile.apps_script_url.trim(), {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        action: targetStatus === "AKTIF" ? "restore_file" : "trash_file",
        spreadsheetId: (storageProfile.spreadsheet_id || "").trim(),
        driveFolderId: (storageProfile.drive_folder_id || "").trim(),
        business_id: business_id || item?.business_id || item?.id,
        timestamp: new Date().toISOString(),
    ...
    try {
      ...
      return res.status(502).json({
        success: false,
        message: "Respons Google Apps Script tidak valid saat ubah status trash.",
      });
    }
...
// Proxy endpoint for BULK PERMANENT DELETE
app.post("/api/storage/bulk-delete", async (req, res) => {
  const { storageProfile, items, user_name } = req.body;
  ...
  try {
    ...
    try {
      ...
      return res.status(502).json({
        success: false,
        message: "Respons Google Apps Script tidak valid saat bulk delete.",
      });
    }
    ...
    return res.status(500).json({
      success: false,
      message: `Gagal bulk delete dari Google Drive/Spreadsheet: ${err.message}`,
    });
  }
});
