...
export default function App() {
  ...
  useEffect(() => {
    ...
    // 2. Fetch Central Global Configuration across all devices (Mobile, Desktop, Laptop)
    AppStore.initGlobalConfig().then((result) => {
      if (result.success && result.isConfigured) {
        refreshAllState();
        // Automatically sync all latest tables from Google Spreadsheet on device boot
        AppStore.syncWithGoogleSpreadsheet().then(() => {
          refreshAllState();
        }).catch(() => {});
      }
    });
