...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case "deleteBankSoal":
        return handlePermanentDeleteFileAndRow(data);

      case "trash_file":
      case "move_to_trash":
      case "moveToTrash":
        return handleTrashStatusChange(data, "TRASH");

      case "restore_file":
      case "restore_from_trash":
      case "restoreFromTrash":
        return handleTrashStatusChange(data, "AKTIF");

      case "bulk_delete":
        return handleBulkPermanentDelete(data);
...
/**
 * 8. TARIK DATA DARI SEMUA SHEET (BANK_SOAL & MASTER TABEL)
 */
function handleFetchAll(data) {
  ...
  try {
    ...
    var tags = fetchTagsData(ss);
    var settings = fetchSettingsData(ss);
    var activities = fetchActivityLogData(ss);
    ...
    return jsonResponse({
      ...
      count: items.length,
      message: "Berhasil memuat " + items.length + " naskah soal dan data master langsung dari Google Spreadsheet!"
    });
  } catch (err) {
    return jsonResponse({
      success: false,
...
/**
 * 10. HAPUS PERMANEN DARI GOOGLE SPREADSHEET & GOOGLE DRIVE
 * Menghapus file fisik di Google Drive serta menghapus baris data di Google Spreadsheet
 */
function handlePermanentDeleteFileAndRow(data) {
  try {
    var businessId = data.business_id || (data.item && data.item.business_id) || data.id;
    var driveFileId = data.drive_file_id || (data.item && (data.item.drive_file_id || data.item.file_id));
    var fileName = data.file_name || (data.item && data.item.file_name);
    var spreadsheetId = data.spreadsheet_id || SPREADSHEET_ID;
    
    var fileDeleted = false;
    var rowDeleted = false;
    var errorDetails = [];

    // 1. Hapus File Fisik di Google Drive jika fileId ada
    if (driveFileId) {
      try {
        var driveFile = DriveApp.getFileById(driveFileId);
        if (driveFile) {
          driveFile.setTrashed(true); // Pindahkan ke trash Google Drive
          fileDeleted = true;
        }
      } catch (driveErr) {
        errorDetails.push("Google Drive: " + driveErr.toString());
      }
    }

    // 2. Buka Spreadsheet & Cari Baris di Sheet BANK_SOAL
    ...
    if (sheet) {
      ...
      if (lastRow >= 2) {
        ...
        // Loop dari baris paling bawah agar tidak merusak index baris saat deleteRow
        for (var i = values.length - 1; i >= 0; i--) {
          var rowBusinessId = String(values[i][0] || "").trim();
          var rowDriveFileId = String(values[i][16] || "").trim(); // Kolom DRIVE_FILE_ID
          var rowFileName = String(values[i][14] || "").trim();

          var isMatch = false;
          if (businessId && rowBusinessId === String(businessId).trim()) {
            isMatch = true;
          } else if (driveFileId && rowDriveFileId === String(driveFileId).trim()) {
            isMatch = true;
          } else if (fileName && rowFileName === String(fileName).trim()) {
            isMatch = true;
          }

          if (isMatch) {
            var targetRow = i + 2; // +2 karena header ada di baris 1, dan 0-indexed
            // Coba ambil drive file id dari baris jika sebelumnya belum terhapus
            if (!fileDeleted && rowDriveFileId) {
              try {
                var f = DriveApp.getFileById(rowDriveFileId);
                if (f) {
                  f.setTrashed(true);
                  fileDeleted = true;
                }
    ...
    // 3. Catat ke Tabel ACTIVITY_LOG
    try {
      var actSheet = ss.getSheetByName("ACTIVITY_LOG");
      if (actSheet) {
        actSheet.appendRow([
          "LOG-" + new Date().getTime(),
          "PERMANENT_DELETE",
          "BankSoal",
          businessId || fileName || "DOC",
          data.title || fileName || "Berkas Bank Soal",
          "Menghapus berkas fisik Google Drive (" + (fileDeleted ? "SUKSES" : "TIDAK ADA") + ") dan baris data Google Spreadsheet (" + (rowDeleted ? "SUKSES" : "TIDAK ADA") + ")",
          data.user_name || "Sistem Admin",
          new Date().toISOString()
        ]);
      }
    } catch (logErr) {}
    ...
    return jsonResponse({
      ...
      message: "Berkas berhasil dihapus secara permanen dari Google Drive dan baris Google Spreadsheet!"
    });
  } catch (err) {
    return jsonResponse({
      success: false,
      message: "Gagal menghapus berkas dari Spreadsheet/Drive: " + err.toString()
    });
  }
}

/**
 * 11. UBAH STATUS TRASH / AKTIF DI SPREADSHEET
 */
function handleTrashStatusChange(data, targetStatus) {
  try {
    var businessId = data.business_id || (data.item && data.item.business_id) || data.id;
    var spreadsheetId = data.spreadsheet_id || SPREADSHEET_ID;
    ...
    var sheet = ss.getSheetByName("BANK_SOAL");
    if (!sheet) {
      return jsonResponse({ success: false, message: "Sheet BANK_SOAL tidak ditemukan" });
    }
    ...
    if (lastRow >= 2) {
      ...
      for (var i = 0; i < values.length; i++) {
        ...
        if (businessId && rowBusinessId === String(businessId).trim()) {
          var rowIndex = i + 2;
          // Kolom 19 (Kolom S) adalah STATUS di sheet BANK_SOAL
          sheet.getRange(rowIndex, 19).setValue(targetStatus);
          // Kolom 25 (Kolom Y) adalah SYNC_STATUS
          sheet.getRange(rowIndex, 25).setValue("SYNCED");
          // Kolom 24 (Kolom X) adalah UPDATED_AT
    ...
    return jsonResponse({
      ...
      updated: updated,
      status: targetStatus,
      message: "Status berkas di Google Spreadsheet berhasil diperbarui menjadi: " + targetStatus
    });
  } catch (err) {
    return jsonResponse({
      success: false,
      message: "Gagal memperbarui status trash di Spreadsheet: " + err.toString()
    });
  }
}

/**
 * 12. HAPUS MASSAL DARI GOOGLE DRIVE & SPREADSHEET
 */
function handleBulkPermanentDelete(data) {
  try {
    ...
    for (var i = 0; i < items.length; i++) {
      var itm = items[i];
      handlePermanentDeleteFileAndRow({
        business_id: itm.business_id || itm.id,
        drive_file_id: itm.drive_file_id || itm.file_id,
    ...
    return jsonResponse({
      ...
      message: "Berhasil membersihkan " + deletedCount + " berkas dari Google Drive & Google Spreadsheet!"
    });
  } catch (err) {
    return jsonResponse({
      success: false,
      message: "Gagal hapus massal: " + err.toString()
    });
  }
}
