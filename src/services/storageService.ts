...
export class StorageService {
  ...
  /**
   * Permanently deletes a file from Google Drive and removes its record row from Google Spreadsheet
   */
  static async deleteBankSoalFromCloud(
    item: BankSoal,
    profile?: StorageProfile
  ): Promise<{ success: boolean; file_deleted?: boolean; row_deleted?: boolean; message: string }> {
    const target = profile || AppStore.getActiveStorageProfile();
    if (!target || !target.apps_script_url) {
    ...
    try {
      const payload = {
        ...
        drive_file_id: item.drive_file_id || (item as any).file_id,
        file_name: item.file_name,
        user_name: AppStore.getCurrentUser().name,
      };
      ...
      return {
        ...
        row_deleted: data?.row_deleted,
        message: data?.message || (data?.success ? "Berkas berhasil dihapus permanen dari Drive & Spreadsheet!" : "Gagal menghapus berkas dari cloud."),
      };
    } catch (err: any) {
      return {
        success: false,
        message: `Gagal sinkronisasi penghapusan ke cloud: ${err.message}`,
      };
    }
  }

  /**
   * Updates Bank Soal status to TRASH in Google Spreadsheet
   */
  static async trashBankSoalInCloud(
    item: BankSoal,
    profile?: StorageProfile
    ...
    try {
      ...
      return {
        success: Boolean(data?.success),
        message: data?.message || "Status trash berhasil disinkronkan ke Spreadsheet",
      };
    } catch (err: any) {
      return {
        success: false,
        message: `Gagal memperbarui status trash di Spreadsheet: ${err.message}`,
      };
    }
  }

  /**
   * Restores Bank Soal status to AKTIF in Google Spreadsheet
   */
  static async restoreBankSoalInCloud(
    item: BankSoal,
    profile?: StorageProfile
  ): Promise<{ success: boolean; message: string }> {
    const target = profile || AppStore.getActiveStorageProfile();
    ...
    try {
      ...
      return {
        success: Boolean(data?.success),
        message: data?.message || "Status dokumen aktif berhasil dipulihkan di Spreadsheet",
      };
    } catch (err: any) {
      return {
        success: false,
        message: `Gagal memulihkan status dokumen di Spreadsheet: ${err.message}`,
      };
    }
  }

  /**
   * Bulk permanent delete items from Google Drive & Google Spreadsheet
   */
  static async bulkDeleteFromCloud(
    items: BankSoal[],
    profile?: StorageProfile
    ...
    try {
      ...
      return {
        success: Boolean(data?.success),
        deleted_count: data?.deleted_count || items.length,
        message: data?.message || `Berhasil membersihkan ${items.length} berkas dari Drive & Spreadsheet!`,
      };
    } catch (err: any) {
      return {
        success: false,
        message: `Gagal bulk delete dari cloud: ${err.message}`,
      };
    }
  }
