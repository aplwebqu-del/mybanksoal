...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const [isPullingSpreadsheet, setIsPullingSpreadsheet] = useState(false);
  ...
  const handlePullFromSpreadsheet = async () => {
    setIsPullingSpreadsheet(true);
    toast.info("Membaca Spreadsheet...", "Mengambil daftar naskah soal langsung dari database Spreadsheet...");
    try {
      const res = await AppStore.syncWithGoogleSpreadsheet();
      if (res.success) {
        toast.success("Spreadsheet Disinkronkan", res.message);
      } else {
        toast.error("Gagal Membaca Spreadsheet", res.message);
      }
    } catch (err: any) {
      toast.error("Kesalahan Koneksi", err.message || "Gagal sinkronisasi");
    } finally {
      setIsPullingSpreadsheet(false);
    }
  };
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 shrink-0 flex-wrap">
              <button
                ...
                disabled={isPullingSpreadsheet}
                title="Tarik seluruh data bank soal langsung dari Google Spreadsheet"
                className="flex items-center gap-2 px-3.5 py-2.5 bg-slate-800/90 hover:bg-slate-750 text-slate-200 border border-slate-700 hover:border-emerald-500/50 font-semibold text-xs sm:text-sm rounded-xl shadow-md transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-50"
              >
                <Database className={`w-4 h-4 text-emerald-400 ${isPullingSpreadsheet ? "animate-spin" : ""}`} />
                <span>{isPullingSpreadsheet ? "Membaca..." : "Tarik Spreadsheet"}</span>
              </button>
