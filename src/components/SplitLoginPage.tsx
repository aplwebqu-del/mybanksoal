...
export const SplitLoginPage: React.FC<SplitLoginPageProps> = ({ onLoginSuccess }) => {
  ...
  useEffect(() => {
    // Load global config and auto-sync users on login page mount
    AppStore.initGlobalConfig().then((res) => {
      if (res.success && res.isConfigured) {
        AppStore.syncWithGoogleSpreadsheet().catch(() => {});
      }
    });
