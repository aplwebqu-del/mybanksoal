...
export const TrashView: React.FC<TrashViewProps> = ({
  ...
  const handleRestore = async (item: BankSoal) => {
    setIsProcessing(true);
    toast.info("Memulihkan Berkas...", `Memulihkan "${item.title}" (${item.business_id}) ke repositori aktif`);
    AppStore.restoreFromTrash(item.id);
    toast.success("Dokumen Dipulihkan", `"${item.title}" kembali ke Bank Soal aktif dan disinkronkan ke Spreadsheet`);
    setIsProcessing(false);
    onRefresh();
  };

  const handlePermanentDelete = async (item: BankSoal) => {
    ...
    if (confirm(`Hapus permanen "${item.title}" (${item.business_id})?\n\nPERINGATAN: Berkas fisik di Google Drive dan baris data di Google Spreadsheet akan ikut terhapus permanen dan tidak dapat dikembalikan.`)) {
      setIsProcessing(true);
      toast.info("Menghapus Permanen...", `Menghapus ${item.business_id} dari Google Drive & Google Spreadsheet...`);
      AppStore.permanentDelete(item.id);
      toast.warning("Dihapus Permanen", `${item.business_id} - ${item.title} telah dihapus dari sistem, Google Drive, dan Spreadsheet`);
      setIsProcessing(false);
      onRefresh();
    }
  };
  ...
  const handleBatchRestore = async () => {
    ...
    toast.success("Pemulihan Berhasil", `${selectedIds.length} berkas dipulihkan ke repositori aktif dan disinkronkan ke Spreadsheet`);
    setSelectedIds([]);
    setIsProcessing(false);
    onRefresh();
  };
  ...
  const handleBatchPermanentDelete = async () => {
    if (selectedIds.length === 0) return;
    if (confirm(`Hapus permanen ${selectedIds.length} berkas yang dipilih?\n\nTindakan ini akan menghapus data selamanya dari Google Drive dan Google Spreadsheet.`)) {
      setIsProcessing(true);
      toast.info("Menghapus Massal...", `Membersihkan ${selectedIds.length} berkas dari Google Drive & Spreadsheet...`);
      selectedIds.forEach((id) => AppStore.permanentDelete(id));
      toast.warning("Dihapus Permanen", `${selectedIds.length} berkas dibersihkan dari database, Google Drive, dan Spreadsheet`);
      setSelectedIds([]);
      setIsProcessing(false);
      onRefresh();
    }
  ...
  const handleEmptyTrash = async () => {
    ...
    if (confirm(`Kosongkan Keranjang Sampah (${trashItems.length} dokumen)?\n\nPERINGATAN: Semua berkas fisik di Google Drive dan catatan di Google Spreadsheet akan dihapus secara permanen!`)) {
      setIsProcessing(true);
      toast.info("Mengosongkan Keranjang Sampah...", "Menghapus semua file fisik dari Google Drive & Spreadsheet...");
      const count = AppStore.emptyTrash();
      toast.warning("Keranjang Sampah Dikosongkan", `${count} dokumen berhasil dihapus permanen dari sistem, Google Drive, dan Spreadsheet`);
      setSelectedIds([]);
      setIsProcessing(false);
      onRefresh();
    }
