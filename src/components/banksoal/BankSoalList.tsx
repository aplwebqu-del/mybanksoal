...
export const BankSoalList: React.FC<BankSoalListProps> = ({
  ...
  // Sync / Read directly from Google Spreadsheet
  const handleSyncFromSpreadsheet = async () => {
    setIsSyncingLive(true);
    toast.info("Membaca Spreadsheet...", "Menghubungkan ke Google Spreadsheet untuk memuat naskah soal...");
    try {
      const res = await AppStore.syncWithGoogleSpreadsheet();
      if (res.success) {
        toast.success("Sinkronisasi Selesai", res.message);
      } else {
        toast.error("Gagal Membaca Spreadsheet", res.message);
      }
    } catch (err: any) {
      toast.error("Kesalahan Koneksi", err.message || "Gagal sinkronisasi data");
    } finally {
  ...
  const handlePermanentDelete = (item: BankSoal, e: React.MouseEvent) => {
    e.stopPropagation();
    setMenuOpenId(null);
    if (
      confirm(
        `HAPUS PERMANEN "${item.title}" (${item.business_id})?\n\nBerkas fisik PDF di Google Drive dan baris data di Google Spreadsheet akan langsung dihapus secara realtime.`
      )
    ) {
      AppStore.permanentDelete(item.id);
      toast.error("Naskah Dihapus Permanen", `${item.business_id} - Sinkron Google Drive & Sheets`);
    }
  };
    ...
    <div id="banksoal-list-view" className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-4 rounded-2xl shadow-sm">
        ...
        <div className="flex items-center gap-2.5 flex-wrap sm:flex-nowrap">
          ...
          {/* Live Sync from Google Spreadsheet Button */}
          <button
            id="list-sync-sheets-btn"
            onClick={handleSyncFromSpreadsheet}
            disabled={isSyncingLive}
            title="Tarik & baca daftar naskah soal langsung dari Google Spreadsheet"
            className="flex items-center gap-1.5 px-3.5 py-2 bg-slate-800 hover:bg-slate-750 text-slate-200 border border-slate-700 hover:border-emerald-500/50 rounded-xl text-xs font-semibold transition-all disabled:opacity-50"
          >
            <RefreshCw className={`w-3.5 h-3.5 text-emerald-400 ${isSyncingLive ? "animate-spin" : ""}`} />
            <span className="hidden sm:inline">
              {isSyncingLive ? "Menarik Data..." : "Tarik Spreadsheet"}
            </span>
          </button>
                          ...
                          <>
                            ...
                            <div className="absolute right-0 mt-1 w-48 bg-slate-850 border border-slate-700 rounded-xl shadow-2xl z-30 p-1 text-xs text-slate-200">
                              ...
                              >
                                ...
                                <span>Pindah ke Sampah</span>
                              </button>
                              <button
                                onClick={(e) => handlePermanentDelete(item, e)}
                                className="w-full flex items-center gap-2 px-3 py-2 text-rose-500 hover:bg-rose-900/60 rounded-lg text-left font-medium"
                              >
                                <Trash2 className="w-3.5 h-3.5 text-rose-500" />
                                <span>Hapus Permanen DB</span>
                              </button>
                            </div>
                          </>
                        )}
