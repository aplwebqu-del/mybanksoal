...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const handleSyncFromSpreadsheet = async () => {
    if (isSyncing) return;
    setIsSyncing(true);
    toast.info("Sinkronisasi Database", "Membaca data terbaru dari Google Spreadsheet...");
    try {
      const result = await AppStore.syncWithGoogleSpreadsheet();
      if (result.success) {
        toast.success("Sinkronisasi Selesai", result.message);
      } else {
        toast.error("Gagal Sinkronisasi", result.message);
      }
    } catch (err: any) {
      toast.error("Gagal Sinkronisasi", err.message || "Terjadi kesalahan koneksi");
    } finally {
      setIsSyncing(false);
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            {/* Connection Status Icon Button: displays green checkmark if connected */}
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl border text-xs font-semibold transition-all hover:scale-[1.02] active:scale-95 ${
                isCloudConnected
                  ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/20 shadow-sm"
                  : "bg-slate-800/90 border-slate-700 text-slate-300 hover:bg-slate-750 hover:text-white shadow-sm"
              }`}
            >
              {isCloudConnected ? (
                <>
            ...
            {/* Live Sync Menu Button (Tarik Data dari Google Spreadsheet) */}
            <button
              id="navbar-sync-spreadsheet-btn"
              onClick={handleSyncFromSpreadsheet}
              disabled={isSyncing}
              title="Tarik data terbaru langsung dari Google Spreadsheet dan sinkronkan ke sistem"
              className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl border text-xs font-semibold transition-all hover:scale-[1.02] active:scale-95 shadow-sm ${
                isSyncing
                  ? "bg-blue-600/30 border-blue-500/40 text-blue-300 cursor-wait"
                  : "bg-slate-800/90 hover:bg-slate-750 border-slate-700 hover:border-emerald-500/40 text-slate-200 hover:text-white"
              }`}
            >
              <RefreshCw className={`w-3.5 h-3.5 text-emerald-400 shrink-0 ${isSyncing ? "animate-spin" : ""}`} />
              <span className="hidden md:inline">{isSyncing ? "Menyinkronkan..." : "Sinkronisasi"}</span>
              <span className="md:hidden">{isSyncing ? "Sync..." : "Sync"}</span>
            </button>
